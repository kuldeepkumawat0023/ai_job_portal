...
// @route   POST /api/v1/user/github-login
// @access  Public
exports.githubLogin = async (req, res, next) => {
  try {
    const { code } = req.body;
    if (!code) {
      return res.status(400).json({ success: false, statusCode: 400, message: 'GitHub authorization code is required', data: null });
    }
    ...
    if (!accessToken) {
      return res.status(401).json({ success: false, statusCode: 401, message: 'Invalid GitHub code', data: null });
    }
    ...
    // 3. Get user email (often private)
    ...
    if (!primaryEmail) {
      return res.status(400).json({ success: false, statusCode: 400, message: 'No verified email found in your GitHub account', data: null });
    }
    ...
    <body>
      <div class="container">
        <div class="content">
          <h2 class="header">AI Job Portal</h2>
          <h3>Welcome Back, ${user.fullname}!</h3>
          <p class="details">We are thrilled to see you again. You have successfully logged into your AI Job Portal account via GitHub.</p>
          <p class="details" style="font-size: 14px; color: #888;">If this was you, you can safely ignore this message.<br>If you did not authorize this login, please contact our support team immediately.</p>
        </div>
      </div>
    </body>
    ...
    sendEmail({
      ...
      subject: 'Welcome Back to AI Job Portal',
      html: loginHtmlMessage
    }).catch(err => console.log('Failed to send GitHub login alert email:', err));
    ...
    return res.status(401).json({ success: false, statusCode: 401, message: 'GitHub authentication failed', data: null });
  }
};
