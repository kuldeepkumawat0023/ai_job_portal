  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        router.push(
          `/verify-otp?email=${normalizedEmail}&type=reset`
        );
      }
    } catch (err: unknown) {
      const message =
    ...
    <AuthSplitLayout>
      ...
      >
        ...
        <h1 className="sr-only">
          Forgot Password - Reset Your AI JobFit
          Account
        </h1>
        ...
        <header className="mb-6 text-center flex flex-col items-center">
          ...
          <p
            ...
            className="sr-only"
          >
            Recover your AI JobFit account securely
            using email verification and OTP reset.
          </p>
        </header>
        ...
        >
          ...
          <div>
            ...
            <div className="relative">
              <Mail
                className={`absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 z-10 transition-colors ${error
                    ? 'text-red-500'
                    : 'text-primary/70'
                  }`}
                aria-hidden="true"
              ...
              <input
                className={`w-full glass-input rounded-xl pl-10 pr-4 py-2.5 text-sm focus:ring-0 transition-all ${error
          ...
          <div className="mt-3 flex items-center justify-center gap-2 px-4 py-1.5 bg-primary/5 backdrop-blur-sm rounded-lg border border-primary/10">
            ...
            <p className="text-[11px] leading-tight text-on-surface-variant italic">
              ...
              Multi-factor verification keeps your
              neural matching profile secure.
            </p>
          </div>
        </form>
