...
/**
 * 🔒 Admin Reactivate Account Form
 * SEO + Accessibility Optimized
 */
const AdminReactivateAccountForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const toastId = toast.loading('Reactivating admin account...');

    try {
      const response = await authService.reactivateAccount({ email, ...formData });

      if (response.success) {
        toast.success('Account reactivated! Welcome back.', { id: toastId });
        router.push('/login');
      }
    } catch (err: unknown) {
  ...
  return (
    <AdminAuthSplitLayout>
      <section className="w-full" aria-labelledby="admin-reactivate-heading">
        {/* SEO Hidden H1 */}
        <h1 className="sr-only">Reactivate AI JobFit Admin Account Securely</h1>
        ...
        <header className="mb-6 text-center flex flex-col items-center">
          ...
          <p id="admin-reactivate-heading" className="sr-only">
            Securely reactivate your AI JobFit Admin account by creating a new password.
          </p>
        </header>
        ...
        <div className="mb-6 text-center">
          <div className="bg-primary/10 w-12 h-12 rounded-2xl flex items-center justify-center mx-auto mb-3 border border-primary/20">
            <RefreshCcw className="text-primary w-6 h-6" aria-hidden="true" />
          </div>

          <h2 className="text-xl font-bold text-primary mb-1">Reactivate Admin Account</h2>

          <p className="text-xs text-on-surface-variant opacity-70">
            Welcome back! Set a new password to restore your admin access.
          </p>
        </div>

        {/* Form */}
        <form onSubmit={handleSubmit} className="space-y-4" aria-label="Admin reactivate account form">
          {/* New Password */}
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                aria-describedby={errors.newPassword ? 'admin-reac-new-password-error' : undefined}
              />
              <button
                type="button"
            ...
            {errors.newPassword && (
              <p id="admin-reac-new-password-error" className="text-[10px] text-red-500 mt-1 font-bold px-1">
                {errors.newPassword}
              </p>
            )}
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                aria-describedby={errors.confirmPassword ? 'admin-reac-confirm-password-error' : undefined}
              />
              <button
                type="button"
            ...
            {errors.confirmPassword && (
              <p id="admin-reac-confirm-password-error" className="text-[10px] text-red-500 mt-1 font-bold px-1">
                {errors.confirmPassword}
              </p>
            )}
          </div>
          ...
          <button
            disabled={loading}
            type="submit"
            aria-label="Reactivate admin account and continue"
          ...
          >
            ...
            {!loading && <CheckCircle2 className="relative z-10 w-4 h-4" aria-hidden="true" />}
          ...
          <div className="mt-3 flex items-center justify-center gap-2 px-4 py-1.5 bg-primary/5 backdrop-blur-sm rounded-lg border border-primary/10">
            ...
            <p className="text-[11px] leading-tight text-on-surface-variant italic">
              ...
              Your profile and permissions will be fully restored after reactivation.
            </p>
          </div>
        </form>
      </section>
...
export default AdminReactivateAccountForm;
